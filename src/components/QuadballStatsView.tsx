...
import { 
  computeAdvancedStats, computeExtendedStats, 
  AdvancedPlayerStats, ExtendedPlayerStats 
...
export default function QuadballStatsView({ players, events, teams, games, seasons }: QuadballStatsViewProps) {
  const [tab, setTab] = useState<'basic' | 'advanced' | 'extended'>('basic');
  ...
  useEffect(() => { setPage(1); }, [search, seasonFilter, teamFilter, positionFilter, minGames, tab]);
  ...
  const data = tab === 'extended' ? filteredExtended : filteredAdvanced;
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex border border-gray-200 rounded-md overflow-hidden text-xs">
            ...
            <button onClick={() => { setTab('extended'); setSortKey('gameScore'); setSortDir('desc'); }}
              className={cn('px-3 py-1 font-medium transition-colors border-l border-gray-200', tab === 'extended' ? 'bg-red-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50')}>
              Extended
            </button>
          </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/80">
                <th className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-left text-gray-400 sticky left-0 bg-gray-50 z-10 min-w-[140px]">Player</th>
                {tab === 'basic' ? (<>
                  ...
                  <SortHeader label="S%" sortKey="shotPct" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Shooting %" />
                  <SortHeader label="A:TO" sortKey="assistToTurnover" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Assist to Turnover" />
                  <SortHeader label="G" sortKey="goals" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Goals" />
                  <SortHeader label="A" sortKey="assists" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Assists" />
                  <SortHeader label="PTS" sortKey="points" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Points" />
                </>) : (<>
                  <SortHeader label="GP" sortKey="gamesPlayed" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Games Played" />
                  <SortHeader label="MIN" sortKey="minutesPlayed" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Minutes Played" />
                  ...
                  <SortHeader label="PTS" sortKey="points" currentSort={sortKey} currentDir={sortDir} onSort={handleSort} tooltip="Points (Total)" />
                </>)}
              </tr>
            </thead>
            <tbody>
              {paged.length === 0 ? (
                ...
                return (
                  <tr key={row.playerId} className="border-b border-gray-50 hover:bg-red-50/30 transition-colors">
                    ...
                    {tab === 'basic' ? (<>
                      ...
                      <Cell value={row.points} bold highlight="pos" />
                    </>) : (<>
                      <Cell value={row.gamesPlayed} />
                      <Cell value={row.minutesPlayed} />
