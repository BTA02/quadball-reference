...
import { RefreshCw, ShieldAlert, PlayCircle, Activity, ChevronDown, ChevronUp } from 'lucide-react';
...
// Events are recorded with `createdAt: new Date().toISOString()`, but rows written by the
// migration scripts can carry a Firestore Timestamp, a Date, or an epoch number instead.
...
export default function RecentEventsView({ games, teams, seasons = [], onOpenGame }: RecentEventsViewProps) {
  ...
  // gameEvents collection has to come down and get filtered here. That's a heavy read for
  // a page most people open to read the docs, so nothing is fetched until it's expanded.
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to load recent events: ' + e.message);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleToggle = () => {
    const next = !expanded;
    setExpanded(next);
    if (next && loadedAt === null && !loading) loadEvents();
  };

  const activity = useMemo<GameActivity[]>(() => {
    if (!loadedAt) return [];
    const cutoff = loadedAt - windowDays * 24 * 60 * 60 * 1000;
    const byGame = new Map<string, { total: number; unverified: number; lastAt: number }>();

    for (const ev of events) {
      const created = parseCreatedAt(ev.createdAt);
      if (created === null || created < cutoff) continue;
      let entry = byGame.get(ev._gameId);
      if (!entry) {
        entry = { total: 0, unverified: 0, lastAt: created };
        byGame.set(ev._gameId, entry);
      }
      entry.total++;
  ...
  }, [events, windowDays, loadedAt]);

  const totalNewStats = useMemo(() => activity.reduce((sum, a) => sum + a.total, 0), [activity]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Games that got new stats recently. Open one to check the new events against the
                video and vote them up or down.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          {loading ? (
            ...
            <div className="p-12 text-center text-gray-400 text-sm">
              No stats have been added in the last {windowDays} days.
            </div>
          ) : (
                  ...
                  return (
                    <tr key={row.gameId} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <p className="font-bold text-gray-900">{row.total.toLocaleString()}</p>
                        {/* The list is ordered by this, so it has to be legible somewhere. */}
