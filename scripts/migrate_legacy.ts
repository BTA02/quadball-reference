...
interface Video { vid_id: string; 
  objectId: string;
  team_id: string;      // Home team
  opponent_id: string;  // Away team
}
...
function runMigration() {
  ...
  const videosArray = loadCsv<Video>(VIDEOS_FILE);
  ...
  const cleanedEvents: Partial<LegacyEvent>[] = [];
  let droppedCount = 0;
  ...
  const eventsByVideo = new Map<string, LegacyEvent[]>();
  for (const ev of stats) {
    if (!eventsByVideo.has(ev.vid_id)) eventsByVideo.set(ev.vid_id, []);
  ...
  for (const [vid_id, gameEvents] of eventsByVideo.entries()) {
    ...
    const homeTeamId = videoContext?.team_id?.trim() || null;
    const awayTeamId = videoContext?.opponent_id?.trim() || null;
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      if (statName === 'START_CLOCK' || statName === 'GAME_START') statName = 'gameStart';
      if (statName === 'PAUSE_CLOCK') statName = 'gamePause';
      if (statName === 'TURNOVER') statName = 'turnover';
      if (statName === 'GAIN_CONTROL') statName = 'control_change';
      if (statName === 'ASSIST') statName = 'assist';
      if (statName === 'SHOT') statName = 'shot';
      if (statName === 'YELLOW_CARD') statName = 'card'; // Should add color='yellow'
      if (statName === 'RED_CARD') statName = 'card';    // Should add color='red'
      if (statName === 'GOAL') statName = 'goal';
      ...
      cleanedEvents.push({
        ...
        stat_name: statName,
        team_id: teamId,
        color: ev.stat_name === 'YELLOW_CARD' ? 'yellow' : ev.stat_name === 'RED_CARD' ? 'red' : undefined
      } as any);
    }
    ...
    if (hasFlagCatch) {
      cleanedEvents.push({
        ...
        vid_id: vid_id,
        fall_year: gameEvents[0]?.fall_year,
